...
type saveStatsReq struct {
	Stats []statRow `json:"stats"`
}

// SaveStats — POST /lobby/:id/stats. Создатель/админ сохраняет статистику матча (K/D/A) игрокам.
func (h *LobbyHandler) SaveStats(c *fiber.Ctx) error {
	ctx, cancel := context.WithTimeout(c.Context(), 10*time.Second)
	defer cancel()
	...
	if !h.canManage(c, l) {
		return fiber.NewError(fiber.StatusForbidden, "сохранять статистику может только создатель или админ")
	}
	var req saveStatsReq
	if err := c.BodyParser(&req); err != nil {
		return fiber.NewError(fiber.StatusBadRequest, "неверный формат запроса")
	}

	stats := h.DB.Collection("match_stats")
	saved := 0
	for _, s := range req.Stats {
		uid, err := primitive.ObjectIDFromHex(s.UserID)
		if err != nil {
			continue // пропускаем несопоставленные строки
		}
		_, err = stats.UpdateOne(ctx,
			bson.M{"lobby_id": l.ID, "user_id": uid},
			bson.M{"$set": bson.M{
				"lobby_id": l.ID, "user_id": uid,
...
// GetStats — GET /lobby/:id/stats. Сохранённая статистика матча.
func (h *LobbyHandler) GetStats(c *fiber.Ctx) error {
	ctx, cancel := context.WithTimeout(c.Context(), 10*time.Second)
	defer cancel()
	...
	cur, err := h.DB.Collection("match_stats").Find(ctx, bson.M{"lobby_id": id})
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, "ошибка базы данных")
	}
	defer cur.Close(ctx)
	var docs []bson.M
	_ = cur.All(ctx, &docs)
	out := make([]fiber.Map, 0, len(docs))
	for _, d := range docs {
		uid, _ := d["user_id"].(primitive.ObjectID)
		out = append(out, fiber.Map{
			"user_id": uid.Hex(), "kills": d["kills"], "deaths": d["deaths"], "assists": d["assists"],
		})
	}
	return c.JSON(fiber.Map{"stats": out})
}
